...
{
    if (argc < 6) {
        std::cerr << "Usage: " << argv[0] << " <num_threads> <num_procs> <is_multiblock> <Nx> <input_folder/> <Nt> \n";
        std::cerr << "  <num_threads> : Number of OpenMP threads (e.g., 14)\n";
        std::cerr << "  <num_procs> : Number of MPI processes (e.g., 3)\n";
    ...
    int num_procs = std::stoi(argv[2]);
    int proc_id;

    mantiis_parallel::launchMPI(proc_id, num_procs, num_threads);

    bool is_multiblock = (std::string(argv[3]) == "true");
    ...
    std::string folder = argv[5]; // "input_output/";
